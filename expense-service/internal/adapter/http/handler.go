...
func SetupRoutes(router *gin.Engine, expenseService *service.ExpenseService) {
	router.POST("/createExpense", func(c *gin.Context) {
	...
	router.DELETE("/deleteExpense/:id", func(c *gin.Context) {
		id := c.Param("id")
	...
	router.POST("/createIncome", func(c *gin.Context) {
		var req model.Expense
		if err := c.ShouldBindJSON(&req); err != nil{
			c.JSON(http.StatusBadRequest, gin.H{"message":"Invalid request"})
			return
	...
	router.DELETE("/deleteIncome/:id", func(c *gin.Context) {
		id := c.Param("id")
		idInt, err := strconv.Atoi(id)
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"message": "Invalid user ID"})
	...
	router.GET("/saludo", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"message":"hola gordito"})
	})
}